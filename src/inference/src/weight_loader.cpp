...
struct GGUFLoader::Impl {
  ...
  bool parse_header(std::string *err) {
    ...
    if (magic != GGUF_MAGIC) {
      ...
      if (kv_count > 2000) { // Safety limit
        *err = "Invalid KV count: " + std::to_string(kv_count);
        return false;
      }

      std::cerr << "[GGUF] Version: " << version
                << ", Tensors: " << tensor_count << ", KV pairs: " << kv_count
                << "\n";
  ...
  bool GGUFLoader::load_tensor(const std::string &name,
    ...
    impl_->file.read(reinterpret_cast<char *>(raw_data.data()),
                     info->size_bytes);
  ...
  bool GGUFLoader::load_tensor_fp16(const std::string &name,
    ...
    if (gtype == GGMLType::F32) {
      ...
      *err =
          "Unsupported type for FP16 load: " + name + " (" + info->dtype + ")";
      return false;
    }
