...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_39"
...
run_one () {
  ...
  local max_seq="$3"
  local rc=0
  local out_jsonl="$OUTDIR/b3_39_wo_resid_${prompt_id}_E1.jsonl"
  ...
  $BIN --model "$MODEL" --prompt-file "$prompt_file" --max-tokens 2 --greedy --debug-decode 2 \
    2>&1 | tee "$OUTDIR/b3_39_E1_${prompt_id}.log"
  rc=${PIPESTATUS[0]}
  set -e
  ...
  if grep -q "Embedding Lookup launch failed" "$OUTDIR/b3_39_E1_${prompt_id}.log"; then
    echo "RUN_FAILED E1 ${prompt_id} embedding_lookup_failed" >&2
    return 2
  fi
  if grep -q "Generation error" "$OUTDIR/b3_39_E1_${prompt_id}.log"; then
    echo "RUN_FAILED E1 ${prompt_id} generation_error" >&2
    return 3
  fi
  if grep -q "hipMalloc" "$OUTDIR/b3_39_E1_${prompt_id}.log"; then
...
tar -czf /root/gretacore_b3_39_artifacts.tgz \
  $OUTDIR/b3_39_E1_*.log \
  $OUTDIR/b3_39_wo_resid_*_E1.jsonl \
  2>/dev/null || true

ls -lh /root/gretacore_b3_39_artifacts.tgz

exit $fail
EOF_REMOTE

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_39_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_39_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_b3_39_wo_resid.py \
  --dir "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE" \
  --out "$OUTDIR_LOCAL/b3_39_analysis.txt"
