...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_41"
...
run_one () {
  ...
  if grep -q "Embedding Lookup launch failed" "$OUTDIR/b3_41_E1_${prompt_id}.log"; then
    echo "RUN_FAILED E1 ${prompt_id} embedding_lookup_failed" >&2
    return 2
  fi
  if grep -q "Generation error" "$OUTDIR/b3_41_E1_${prompt_id}.log"; then
    echo "RUN_FAILED E1 ${prompt_id} generation_error" >&2
    return 3
  fi
  if grep -q "hipMalloc" "$OUTDIR/b3_41_E1_${prompt_id}.log"; then
...
unset GRETA_TRACE_POST_WO_OUT

mkdir -p /root

tar -czf /root/gretacore_b3_41_artifacts.tgz \
  $OUTDIR/b3_41_E1_*.log \
  $OUTDIR/b3_41_post_wo_*_E1.jsonl \
  2>/dev/null || true

ls -lh /root/gretacore_b3_41_artifacts.tgz

exit $fail
EOF_REMOTE

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_41_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_41_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_b3_41_post_wo.py \
  --dir "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE" \
  --out "$OUTDIR_LOCAL/b3_41_analysis.txt"
