...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_29"
...
export GRETA_TRACE_STAGE_OUT=$OUTDIR/b3_29_stage.jsonl
export GRETA_TRACE_STAGE_LAYERS="0,1,2,15,31"
export GRETA_TRACE_STAGE_POINTS="x_in,attn_out,x_after_attn,mlp_out,x_out,final_rms,lm_head_in,logits"
...
tar -czf /root/gretacore_b3_29_artifacts.tgz \
  $OUTDIR/b3_29_stage.jsonl \
  $OUTDIR/b3_29_p4.log \
  $OUTDIR/b3_29_p5.log

ls -lh /root/gretacore_b3_29_artifacts.tgz
EOF_REMOTE

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_29_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_29_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_stage_trace.py \
  "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE/b3_29_stage.jsonl" \
  --out "$OUTDIR_LOCAL/b3_29_analysis.txt"
