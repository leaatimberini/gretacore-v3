...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_38"
...
mkdir -p "$OUTDIR"
rm -f "$OUTDIR"/b3_38_* || true

export GRETA_INT4_WEIGHTS=1
export GRETA_TRACE_ATTN_L0_PIPE=1
export GRETA_TRACE_STAGE=1
export GRETA_TRACE_STAGE_LAYERS="0"
export GRETA_TRACE_STAGE_POINTS="attn_out,x_after_attn,ffn_norm,mlp_out,x_after_mlp,x_out,final_norm,lm_head_in,logits"
...
run_one () {
  ...
  local max_seq="$3"
  local rc=0
  local out_jsonl="$OUTDIR/b3_38_postqkv_${prompt_id}_E1.jsonl"

  export GRETA_MAX_SEQ_LEN="$max_seq"
  export GRETA_TRACE_PROMPT_ID="$prompt_id"
  export GRETA_TRACE_ATTN_L0_PIPE_OUT="$out_jsonl"
  ...
  $BIN --model "$MODEL" --prompt-file "$prompt_file" --max-tokens 2 --greedy --debug-decode 2 \
    2>&1 | tee "$OUTDIR/b3_38_E1_${prompt_id}.log"
  rc=${PIPESTATUS[0]}
  set -e
  ...
  if grep -q "Embedding Lookup launch failed" "$OUTDIR/b3_38_E1_${prompt_id}.log"; then
    echo "RUN_FAILED E1 ${prompt_id} embedding_lookup_failed" >&2
    return 2
  fi
  if grep -q "Generation error" "$OUTDIR/b3_38_E1_${prompt_id}.log"; then
    echo "RUN_FAILED E1 ${prompt_id} generation_error" >&2
    return 3
  fi
  if grep -q "hipMalloc" "$OUTDIR/b3_38_E1_${prompt_id}.log"; then
...
tar -czf /root/gretacore_b3_38_artifacts.tgz \
  $OUTDIR/b3_38_E1_*.log \
  $OUTDIR/b3_38_postqkv_*_E1.jsonl \
  2>/dev/null || true

ls -lh /root/gretacore_b3_38_artifacts.tgz

exit $fail
EOF_REMOTE

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_38_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_38_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_b3_38_postqkv.py \
  --dir "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE" \
  --out "$OUTDIR_LOCAL/b3_38_analysis.txt"
