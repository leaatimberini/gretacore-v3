...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_25"
...
export GRETA_TRACE_ATTN_OUT=$OUTDIR/b3_25_attn_vacc.jsonl
...
rm -f "$OUTDIR/b3_25_attn_vacc.jsonl"

export GRETA_TRACE_PROMPT_ID=p4_sys
export GRETA_TRACE_PREFILL_DECODE_OUT=$OUTDIR/b3_25_p4_prefill_decode.jsonl
$BIN --model "$MODEL" --prompt "$P4" --max-tokens 2 --greedy --debug-decode 1 \
  2>&1 | tee "$OUTDIR/b3_25_p4.log"

export GRETA_TRACE_PROMPT_ID=p5_ba
export GRETA_TRACE_PREFILL_DECODE_OUT=$OUTDIR/b3_25_p5_prefill_decode.jsonl
$BIN --model "$MODEL" --prompt "$P5" --max-tokens 2 --greedy --debug-decode 1 \
  2>&1 | tee "$OUTDIR/b3_25_p5.log"

unset GRETA_TRACE_PROMPT_ID

tar -czf /root/gretacore_b3_25_artifacts.tgz \
  $OUTDIR/b3_25_attn_vacc.jsonl \
  $OUTDIR/b3_25_p4_prefill_decode.jsonl \
  $OUTDIR/b3_25_p5_prefill_decode.jsonl \
  $OUTDIR/b3_25_p4.log \
  $OUTDIR/b3_25_p5.log

ls -lh /root/gretacore_b3_25_artifacts.tgz
EOS

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_25_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_25_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_b3_25_vacc.py \
  --vacc-jsonl "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE/b3_25_attn_vacc.jsonl" \
  --p4-jsonl "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE/b3_25_p4_prefill_decode.jsonl" \
  --p5-jsonl "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE/b3_25_p5_prefill_decode.jsonl" \
  --out "$OUTDIR_LOCAL/b3_25_analysis.txt"
