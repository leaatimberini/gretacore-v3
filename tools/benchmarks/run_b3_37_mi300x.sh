...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_37"
...
run_one () {
  ...
  export GRETA_TRACE_PREFILL_DECODE_OUT="$OUTDIR/b3_37_${exp}_prefill_decode_${prompt_id}.jsonl"
  rm -f "$OUTDIR/b3_37_${exp}_prefill_decode_${prompt_id}.jsonl"
  set +e
  $BIN --model "$MODEL" --prompt-file "$prompt_file" --max-tokens 16 --greedy --debug-decode 16 \
    2>&1 | tee "$OUTDIR/b3_37_${exp}_${prompt_id}.log"
  rc=${PIPESTATUS[0]}
  set -e
  if [[ $rc -ne 0 ]]; then
  ...
  if grep -q "Embedding Lookup launch failed" "$OUTDIR/b3_37_${exp}_${prompt_id}.log"; then
    echo "RUN_FAILED ${exp} ${prompt_id} embedding_lookup_failed" >&2
    return 2
  fi
  if grep -q "Generation error" "$OUTDIR/b3_37_${exp}_${prompt_id}.log"; then
    echo "RUN_FAILED ${exp} ${prompt_id} generation_error" >&2
    return 3
  fi
  if grep -q "hipMalloc" "$OUTDIR/b3_37_${exp}_${prompt_id}.log"; then
    echo "RUN_FAILED ${exp} ${prompt_id} hipMalloc_error" >&2
    return 5
  fi
  if [[ ! -s "$OUTDIR/b3_37_${exp}_prefill_decode_${prompt_id}.jsonl" ]]; then
...
run_matrix () {
  ...
  run_one "$exp" p0_short /root/gretacore/tools/benchmarks/prompts/p0_short.txt 256 || fail=1
...
tar -czf /root/gretacore_b3_37_artifacts.tgz \
  $OUTDIR/b3_37_*.log \
  $OUTDIR/b3_37_*_prefill_decode_*.jsonl \
  2>/dev/null || true

ls -lh /root/gretacore_b3_37_artifacts.tgz

exit $fail
EOF_REMOTE

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_37_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_37_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_b3_37_acceptance.py \
  --dir "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE" \
  --out "$OUTDIR_LOCAL/b3_37_analysis.txt"
