...
def main():
    ...
    base = Path(args.dir)
    files = sorted(base.glob("b3_36_E*_attn_l0_pipe_*.jsonl"))
    lines = []
    lines.append(f"B3.36 prefill Wv layout fix analysis: {base}")
    lines.append(
    ...
    for path in files:
        name = path.name
        try:
            exp = name.split("_attn_l0_pipe_")[0].split("b3_36_")[1]
        ...
        pre_layout_v = pre.get("v_weight_layout_best", "unknown")
        dec_layout_v = dec.get("v_weight_layout_best", "unknown")
        v_mae = mae(pre.get("v", []), dec.get("v", []))
        k_mae = mae(pre.get("k", []), dec.get("k", []))
        q_mae = mae(pre.get("q", []), dec.get("q", []))
        first, _ = first_mismatch(pre, dec)
        lines.append(
            f"{prompt}\t{exp}\t{pre_layout_v}\t{dec_layout_v}\t{first}\t{v_mae}\t{k_mae}\t{q_mae}"
        )
