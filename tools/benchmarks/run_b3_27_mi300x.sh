...
OUTDIR_LOCAL="$ROOT_DIR/artifacts_remote/${DATE}/b3_27"
...
export GRETA_TRACE_STAGE_OUT=$OUTDIR/b3_27_stage.jsonl
export GRETA_TRACE_STAGE_LAYERS="0,1,2,15,31"
export GRETA_TRACE_STAGE_POINTS="attn_out,x_after_attn,mlp_out,x_after_mlp,final_norm,lm_head_in,logits,x_in"
...
rm -f "$OUTDIR/b3_27_stage.jsonl"

export GRETA_TRACE_PROMPT_ID=p4_sys
$BIN --model "$MODEL" --prompt "$P4" --max-tokens 2 --greedy --debug-decode 1 \
  2>&1 | tee "$OUTDIR/b3_27_p4.log"

export GRETA_TRACE_PROMPT_ID=p5_ba
$BIN --model "$MODEL" --prompt "$P5" --max-tokens 2 --greedy --debug-decode 1 \
  2>&1 | tee "$OUTDIR/b3_27_p5.log"

unset GRETA_TRACE_PROMPT_ID

tar -czf /root/gretacore_b3_27_artifacts.tgz \
  $OUTDIR/b3_27_stage.jsonl \
  $OUTDIR/b3_27_p4.log \
  $OUTDIR/b3_27_p5.log

ls -lh /root/gretacore_b3_27_artifacts.tgz
EOF

mkdir -p "$OUTDIR_LOCAL"
scp root@"$IP":/root/gretacore_b3_27_artifacts.tgz "$OUTDIR_LOCAL/"

tar -xzf "$OUTDIR_LOCAL/gretacore_b3_27_artifacts.tgz" -C "$OUTDIR_LOCAL/"

python3 tools/benchmarks/analyze_b3_27_stage.py \
  --jsonl "$OUTDIR_LOCAL/root/gretacore/artifacts/alignment/$DATE/b3_27_stage.jsonl" \
  --out "$OUTDIR_LOCAL/b3_27_analysis.txt"
