...
// FP16 (IEEE 754 half) encode/decode minimal (host side)
static uint16_t f32_to_f16(float x) {
  // Conversor simple (no ultra-preciso) suficiente para bench.
  // Basado en bit tricks IEEE754.
  uint32_t u;
...
int main(int argc, char **argv) {
  ...
  std::cout << "GRETA CORE Runtime Bench: vk_gemm_f16acc32_tiled_ts_bench\n";
  ...
  std::vector<uint16_t> Aref(M * K);
  std::vector<uint16_t> Bref(K * N);
  if (!gcore::rt::vk::read_device_to_host(
          dev, pool, q, bufA, stageA, bytesA,
          [&](const void *ptr, VkDeviceSize sz) {
            std::memcpy(Aref.data(), ptr, static_cast<size_t>(sz));
          },
          &err) ||
      !gcore::rt::vk::read_device_to_host(
          dev, pool, q, bufB, stageB, bytesB,
          [&](const void *ptr, VkDeviceSize sz) {
            std::memcpy(Bref.data(), ptr, static_cast<size_t>(sz));
          },
          &err)) {
    std::cerr << "read_device_to_host failed (A/B): " << err << "\n";
    validation_ok = false;
  } else {
    cpu_ref_8x8_f16(Aref.data(), Bref.data(), Cref.data(), M, N, K, lda, ldb,
                    ldc);
  }
  ...
  if (!gcore::rt::vk::read_device_to_host(
          ...
          &err)) {
    std::cerr << "read_device_to_host failed (C): " << err << "\n";
    validation_ok = false;
  }
  if (validation_ok) {
    bool ok = true;
    ...
    for (int r0 = 0; r0 < rm; r0++) {
      for (int c0 = 0; c0 < cn; c0++) {
        ...
        if (absd > 1e-1)
          ok = false; // FP16 tolerancia mayor
      }
    }
  ...
  if (!kernel_ms.empty()) {
    std::vector<double> km = kernel_ms;
    ...
    std::cout << "RESULT vk_gemm_f16acc32_tiled_ts_bench (GPU-only kernel):\n";
