...
int main(int argc, char **argv) {
  ...
  std::cout
      << "GRETA CORE Runtime Bench: vk_gemm_f16acc32_tiled_vec2_db_ts_bench\n";
  ...
  std::filesystem::path spv_path = std::filesystem::current_path() / "build" /
                                   "gemm_f16acc32_tiled_vec2_db.comp.spv";
  ...
  if (!kernel_ms.empty()) {
    ...
    std::cout << "RESULT vk_gemm_f16acc32_tiled_vec2_db_ts_bench (GPU-only "
                 "kernel):\n";
