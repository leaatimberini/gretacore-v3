...
int main(int argc, char **argv) {
  ...
  std::cout << "GRETA CORE Runtime Bench: vk_layernorm_tiled_bench\n";
  ...
  gcore::rt::vk::Buffer x_dev{}, g_dev{}, b_dev{}, y_dev{};
  gcore::rt::vk::Buffer x_stage{}, g_stage{}, b_stage{}, y_stage{};
  auto cleanup = [&]() {
    gcore::rt::vk::destroy_buffer(dev, &y_stage);
    gcore::rt::vk::destroy_buffer(dev, &b_stage);
    gcore::rt::vk::destroy_buffer(dev, &g_stage);
    gcore::rt::vk::destroy_buffer(dev, &x_stage);
    gcore::rt::vk::destroy_buffer(dev, &y_dev);
    gcore::rt::vk::destroy_buffer(dev, &b_dev);
    gcore::rt::vk::destroy_buffer(dev, &g_dev);
  ...
  if (!gcore::rt::vk::create_device_local_buffer(
      ...
      !gcore::rt::vk::create_device_local_buffer(
          phys, dev, bytes_g, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, &g_dev,
          &err) ||
      !gcore::rt::vk::create_device_local_buffer(
          phys, dev, bytes_g, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, &b_dev,
          &err) ||
      !gcore::rt::vk::create_device_local_buffer(
      ...
      !gcore::rt::vk::create_staging_buffer(phys, dev, bytes_x, &x_stage,
                                            &err) ||
      !gcore::rt::vk::create_staging_buffer(phys, dev, bytes_g, &g_stage,
                                            &err) ||
      !gcore::rt::vk::create_staging_buffer(phys, dev, bytes_g, &b_stage,
                                            &err) ||
  ...
  auto upload_g = [&](void *ptr, VkDeviceSize sz) {
    auto *dst = reinterpret_cast<float *>(ptr);
    size_t n = size_t(sz / 4);
    for (size_t i = 0; i < n; i++)
      dst[i] = 1.0f;
  };
  auto upload_b = [&](void *ptr, VkDeviceSize sz) {
    std::memset(ptr, 0, size_t(sz));
  };

  if (!gcore::rt::vk::stage_host_to_device(dev, pool, q, x_stage, x_dev, bytes_x,
                                           upload_x, &err) ||
      !gcore::rt::vk::stage_host_to_device(dev, pool, q, g_stage, g_dev, bytes_g,
                                           upload_g, &err) ||
      !gcore::rt::vk::stage_host_to_device(dev, pool, q, b_stage, b_dev, bytes_g,
                                           upload_b, &err)) {
  ...
  dbi[1] = {g_dev.buf, 0, bytes_g};
  dbi[2] = {b_dev.buf, 0, bytes_g};
  dbi[3] = {y_dev.buf, 0, bytes_x};
  ...
  if (timing_ok) {
    ...
    std::cout << "RESULT vk_layernorm_tiled_bench: mean_ms=" << mean
  ...
  std::vector<float> g_host(cols, 1.0f);
  std::vector<float> b_host(cols, 0.0f);
  if (!gcore::rt::vk::read_device_to_host(
  ...
  layernorm_ref(x_host.data(), ref.data(), g_host.data(), b_host.data(), rows,
                cols, eps);
  ...
  bool validation_ok = max_abs < 5e-3;
  std::cout << "VALIDATION(layernorm_tiled): "
            << (validation_ok ? "OK" : "FAILED") << " max_abs_err=" << max_abs
            << "\n";
