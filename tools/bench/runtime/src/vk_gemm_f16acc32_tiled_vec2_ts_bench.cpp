...
int main(int argc, char **argv) {
  // N debe ser múltiplo de 2 para el mapping col0=gid.x*2
  ...
  std::cout
      << "GRETA CORE Runtime Bench: vk_gemm_f16acc32_tiled_vec2_ts_bench\n";
  ...
  std::filesystem::path spv_path = std::filesystem::current_path() / "build" /
                                   "gemm_f16acc32_tiled_vec2.comp.spv";
  ...
  // gx se divide por 2 porque cada hilo produce 2 columnas
  const uint32_t lx = 16, ly = 16;
  const uint32_t gx = (uint32_t((N + 1) / 2) + lx - 1) / lx;
  ...
  if (gcore::rt::vk::read_device_to_host(
    ...
    for (int r0 = 0; r0 < rm; r0++) {
      for (int c0 = 0; c0 < cn; c0++) {
        float got = hostC[size_t(r0) * size_t(ldc) + size_t(c0)];
        float exp = Cref[size_t(r0) * size_t(ldc) + size_t(c0)];
  ...
  if (!kernel_ms.empty()) {
    ...
    // FLOPs reales: seguimos calculando GEMM M*N*K*2 (aunque cada hilo haga 2
    // cols)
    ...
    std::cout
        << "RESULT vk_gemm_f16acc32_tiled_vec2_ts_bench (GPU-only kernel):\n";
